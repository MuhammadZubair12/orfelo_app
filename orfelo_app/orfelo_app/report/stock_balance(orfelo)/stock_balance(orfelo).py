	...
	for group_by_key in iwb_map:
		...
		if item_map.get(item):
			...
			report_data = {
				...
				"warehouse": warehouse,
				"company": company,
				"stock_uom_conversion": stock_uom_conversion,
...
def get_columns(filters: StockBalanceFilter):
	...
	columns.extend(
		[
			...
			{
				"label": _("Stock UOM Conversion"),
				"fieldname": "stock_uom_conversion",
				"fieldtype": "Data",
				"width": 140,
			},
			{
				"label": _("UOM Balance Qty"),
				"fieldname": "uom_bal_qty",
				"fieldtype": "Data",
				"width": 140,
			},
			{
	...
	for d in sle:
		...
		if d.posting_date < from_date or (
			...
			if flt(qty_diff, float_precision) >= 0:
				...
				qty_dict.uom_in_qty += qty_diff/dop[0][1]
				qty_dict.in_val += value_diff
			else:
				qty_dict.out_qty += abs(qty_diff)
				qty_dict.uom_out_qty += abs(qty_diff)/dop[0][1]
				qty_dict.out_val += abs(value_diff)

		qty_dict.val_rate = d.valuation_rate
		if dop:
			qty_dict.uom_bal_qty += qty_diff/dop[0][1]
			# qty_dict.stock_uom_conversion = 7
		else:
			qty_dict.uom_bal_qty += 0
